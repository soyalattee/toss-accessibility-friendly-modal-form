...
interface FormModalProps extends BaseModalProps {
  title: string;
  description?: string;
  initialData?: { name: string; email: string; specialty: string };
}

export const LongFormModal = ({
  title,
  description,
  ...
  const validateForm = () => {
    ...
    if (!formData.specialty.trim()) {
      newErrors.specialty = '특이사항을 입력해주세요.';
    }
        ...
        <div style={{ marginBottom: '16px' }}>
          ...
          <input
            ...
            style={{
              width: '100%',
              padding: '8px',
              border: `1px solid ${errors.specialty ? '#ef4444' : '#d1d5db'}`,
              borderRadius: '4px',
              fontSize: '14px',
            }}
